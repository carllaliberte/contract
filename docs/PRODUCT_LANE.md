...
| META token dashboard (`google-app/`, `meta.sol`) | **Archived surface** (display name **ClapShot**). No new product work. |
| Dependabot on `google-app` | **Removed**. Do not merge leftover PRs. |
| Parallel agent tracks | **Forbidden**. One owner, one lane. |

Brand canon: [`docs/BRAND.md`](BRAND.md).
...
- Shipping a token, wallet, or Play listing for META while Clapshot is the face
...
The GitHub repo is still `carllaliberte/contract` for history. That name is not the product name. The product is Clapshot. Do not rename the repo in this lane.
